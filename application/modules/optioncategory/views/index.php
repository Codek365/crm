            ...
            <div class="portlet light">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-green-sharp bold uppercase">Quản lý danh sách Danh Mục Tùy Chọn</span>
                    </div>
                    <div class="tools">
                ...
                <div class="portlet-body">
                    ...
                    <form id="form" action="<?php echo base_url(); ?>optioncategory/delete" method="post">
                    <div id="table">
                        ...
                        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample_1_info">
                                ...
                                <tr class="filter">
                                    ...
                                    <td>
                                        <select name="filter_status">
                                            <option value="*"></option>
                                            <option value="1" <?php if(isset($filter['status']) && $filter['status'] == "1"){echo 'selected';} ?>>Bật</option>
                                            <option value="0" <?php if(isset($filter['status']) && $filter['status'] == "0"){echo 'selected';} ?>>Tắt</option>
                                        </select>
                                    </td>
                                    <td style="width:9%;"></td>
                                </tr>
                            <?php if($list) {
                                ...
                                <tr class="gradeX <?php if($key%2==0){echo 'odd active';}else{echo 'even active';} ?>" role="row">
                                    <td>
                                        <input type="checkbox" id="check"  name="case[]" value="<?php echo $value['option_category_id']; ?>">
                                    </td>
                                    <td><?php echo $value['name']; ?></td>
                                    <td><?php if($value['date_start'] && $value['date_start'] != '1970-01-01'){echo date('d-m-Y',strtotime($value['date_start']));} ?></td>
                                    <td><?php if($value['date_end'] && $value['date_start'] != '1970-01-01'){echo date('d-m-Y',strtotime($value['date_end']));} ?></td>
                                    <td><?php if($value['status'] == 1){echo "Bật";}else{echo "Tắt";} ?></td>
                                    <td style="text-align: center;">
                                        <a href="<?php echo site_url('optioncategory'); ?>/edit/<?php echo $value['option_category_id']; ?>">Sửa <i class="fa fa-edit"></i></a>
                                    </td>
                                </tr>
                                <?php }}else{?>
                                <tr>
...
<script type="text/javascript">
    ...
    $('#btnAdd').on('click',function(){
        location = '<?php echo base_url(); ?>optioncategory/create';
    });
    $('#search').on('input',function(){
        $.ajax({
            url: '<?php echo base_url(); ?>optioncategory/search',
            type: 'post',
            dataType: 'html',
    ...
    function loaddatatable(){
        ...
        $.ajax({
            ...
            type: 'post',
            dataType: 'html',
            data:{filter_row:table_row_length,filter_name:filter_name,filter_status:filter_status},
            success: function(results) {
