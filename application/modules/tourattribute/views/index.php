            ...
            <div class="portlet light">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-green-sharp bold uppercase">Quản lý danh sách Thuộc Tính Tour</span>
                    </div>
                    <div class="tools">
                ...
                <div class="portlet-body">
                    ...
                    <form id="form" action="<?php echo base_url(); ?>tourattribute/delete" method="post">
                    <div id="table">
                    <div class="table-scrollable">
                        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample_1_info">
                            ...
                            <tbody>
                                <tr class="filter">
                                    <td style="width:4%;"></td>
                                    <td><input type="text" name="filter_attribute" value="<?php if(isset($filter['attribute'])){echo $filter['attribute'];} ?>" style="width:100%;" /></td>
                                    <td><input type="text" value="<?php if(isset($filter['name_group'])){echo $filter['name_group'];} ?>" id="dp1" name="filter_name_group" style="width:100%;" /></td>
                                    <td></td>
                                    <td style="width:7%;"></td>
                                </tr>
                            <?php if($list) {
                                ...
                                <tr class="gradeX <?php if($key%2==0){echo 'odd active';}else{echo 'even active';} ?>" role="row">
                                    <td>
                                        <input type="checkbox" id="check"  name="case[]" value="<?php echo $value['attribute_id']; ?>">
                                    </td>
                                    <td><?php echo $value['name']; ?></td>
                                    <td><?php echo $value['name_group']; ?></td>
                                    <td><?php echo $value['sort_order']; ?></td>
                                    <td style="text-align: center;">
                                        <a href="<?php echo site_url('tourattribute'); ?>/edit/<?php echo $value['attribute_id']; ?>">Sửa <i class="fa fa-edit"></i></a>
                                    </td>
                                </tr>
                                <?php }}else{?>
                                <tr>
...
<script type="text/javascript">
    ...
    $('#btnAdd').on('click',function(){
        location = '<?php echo base_url(); ?>tourattribute/create';
    });
    $('#search').on('input',function(){
        $.ajax({
            url: '<?php echo base_url(); ?>tourattribute/search',
            type: 'post',
            dataType: 'html',
    ...
    function loaddatatable(){
        ...
        var filter_name_group = $('input[name=\'filter_name_group\']').val();
        var url = location.href;
        $.ajax({
            url: url,
            type: 'post',
            dataType: 'html',
            data:{filter_row:table_row_length,filter_attribute:filter_attribute,filter_name_group:filter_name_group},
            success: function(results) {
