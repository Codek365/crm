            ...
            <div class="portlet light">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-green-sharp bold uppercase">Quản lý Danh Sách Tùy Chọn</span>
                    </div>
                    <div class="tools">
                ...
                <div class="portlet-body">
                    ...
                    <form id="form" action="<?php echo base_url(); ?>option/delete" method="post">
                    <div id="table">
                        ...
                        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="sample_1_info">
                                    ...
                                    <td>
                                        <select name="filter_category">
                                            ...
                                            <?php if(isset($list_category)){foreach ($list_category as $value) { ?>
                                                <option value="<?php echo $value['option_category_id']; ?>" <?php if(isset($filter['category']) && $filter['category'] == $value['option_category_id']){echo 'selected';} ?>><?php echo $value['name']; ?></option>
                                            <?php }} ?>
                                        </select>
                                    </td>
                                    <td>
                                        <select name="filter_class">
                                            ...
                                            <?php if(isset($list_class)){foreach ($list_class as $value) { ?>
                                                <option value="<?php echo $value['option_class_id']; ?>" <?php if(isset($filter['class']) && $filter['class'] == $value['option_class_id']){echo 'selected';} ?>><?php echo $value['name']; ?></option>
                                            <?php }} ?>
                                        </select>
                                    </td>
                                    <td style="width:7%;"></td>
                            ...
                            <?php if($list) {
                                ...
                                <tr class="gradeX <?php if($key%2==0){echo 'odd active';}else{echo 'even active';} ?>" role="row">
                                    <td>
                                        <input type="checkbox" id="check"  name="case[]" value="<?php echo $value['option_id']; ?>">
                                    </td>
                                    ...
                                    <td style="text-align: center;">
                                        <a href="<?php echo site_url('option'); ?>/edit/<?php echo $value['option_id']; ?>">Sửa <i class="fa fa-edit"></i></a>
                                    </td>
                                </tr>
                                <?php }}else{?>
                                <tr>
...
<script type="text/javascript">
    ...
    $('#btnAdd').on('click',function(){
        location = '<?php echo base_url(); ?>option/create';
    });
    $('#search').on('input',function(){
        $.ajax({
            url: '<?php echo base_url(); ?>option/search',
            type: 'post',
            dataType: 'html',
    ...
    function loaddatatable(){
        ...
        $.ajax({
            ...
            type: 'post',
            dataType: 'html',
            data:{filter_row:table_row_length,filter_name:filter_name,filter_category:filter_category,filter_class:filter_class},
            success: function(results) {
